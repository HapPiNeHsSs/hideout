...
class Character():
    def __init__(self, name, tcp, main, fbdat):
        self.name  = name
        self.tcp = tcp
        self.fbdat = fbdat
        ...
        self.handlers = { "disconnect":self.connectionLost,
                          ...
                          "fbinvite":self.fbinvite,
                          "fbinvitelist":self.fbinviteList,
                          "fbinviteaccept":self.fbinviteAccept,
                          "setscore":self.setScore,
                          "gethiscore":self.getHiscore,
                          "questhelpaccept":self.questHelpAccept,
                          "questhelp":self.questHelp}
    ...
    def sendOffline(self):
        print "ALALA", self.main.chatStore, self.name.__str__()
        offlineMessages = self.main.chatStore.get(self.name.__str__())
        self.main.chatStore.delete(self.name.__str__())
        if not offlineMessages == None:
            for mes in str(offlineMessages).split("|-|"):
                print "DEBBBB", offlineMessages;
                if mes!='':
                    self.reply(mes)
            ...
            try:
                ...
                self.room.getAllPlayerAttributes().items():
                    room_upd = json.dumps({"room_upd":\
                                              {"player":\
                                                  {player:attributes},\
                                               "fb":self.main.players[player].fbdat
                                              }
                                           }) 
                    self.reply(room_upd)
                ...
                room_upd_others = json.dumps({'room_upd':\
                                              ...
                                              "fb":self.main.players[self.name].fbdat
                                            }})
    ...
    def sendNews(self, to, message, _type="news",\
                 _apn = "A News has been delivered"):
        _from = self.name            
        payload={_type:{"from":_from,"message":message}}
                ...
                if type(result).__name__ == "str":
                    ...
                    if cacheResult == None:
                        self.main.chatStore.set(to.__str__()\
                                                ,"|-|%s"%mes)
                        print "DIB: Stored", to.__str__()
                    else:
                        self.main.chatStore.append\
                                (to.__str__(),"|-|%s"%mes)
                        print "DIB: Added", to.__str__()
                    self.main.apnSend(to, _apn)
                elif result == -1:
...
#    def fbinvite(self, val):
#        if val.has_key("from") and val.has_key("fbfrom") and \
#          len(val.keys())==2:
#            self.main.fbinvite(val["from"], val["to"])
#    
#    def fbinviteList(self, val):
#        if val.has_key("from") and len(val.keys())==1:
#            self.main.fbinvite(val["from"])
#    
#    def fbinviteAccept(self, val):
#        if val.has_key("from") and val.has_key("to") and \
#          len(val.keys())==2:
#            self.main.fbinviteAccept(val["to"], val["from"])

    def fbinvite(self, val):
        if val.has_key("from") and val.has_key("fb") and \
           val.has_key("fbto") and len(val.keys())==3:
            self.main.fbinvite(val["from"], val["fb"], val["fbto"])
    
    def fbinviteList(self, val):
        if val.has_key("from") and val.has_key("fb") and len(val.keys())==2:
            self.main.fbinviteList(val["from"], val["fb"])
    
    def fbinviteAccept(self, val):
        if val.has_key("from") and val.has_key("fb") and val.has_key("accept") \
          and len(val.keys())==3:
            self.main.fbinviteAccept(val["from"], val["accept"], val["fb"])
    
    def setScore(self, val):
        if val.has_key("from") and val.has_key("score")\
          and val.has_key("quest") and len(val.keys())==3:    
            self.main.insertHiscore(val["quest"],\
                                    self.name,\
                                    self.fbdat,\
                                    val["score"])
            dat = {"setscore":"success"}
            self.reply(json.dumps(dat))
    
    def questHelp(self, val):
        if val.has_key("from") and val.has_key("to")\
          and val.has_key("quest") and len(val.keys())==3:
            self.sendNews(val["to"], val["quest"], "questhelp",\
                              "A Quest Help has been received.")
        
            dat = {"questhelp":"success"}
            self.reply(json.dumps(dat))
    
    def questHelpAccept(self, val):
        if val.has_key("from") and val.has_key("to")\
          and val.has_key("quest") and len(val.keys())==3:
            self.sendNews(val["to"], val["quest"], "questhelpaccept",\
                              "A Quest Help has been accepted.")
            dat = {"questhelpaccept":"success"}
            self.reply(json.dumps(dat))
    
    def getHiscore(self, val):
        def cbRes(res):
            self.score = res
            mes = {"gethiscore":self.score}
            self.reply(json.dumps(mes))
        def cbError(err_):
            trap = err_.trap(NoCurrentExceptionError, OperationalError)
            if trap == NoCurrentExceptionError:
                ...
                msg("MySQL No connection, traceback: %s"%err_)
                mes = {"gethiscore":{"error":"Database Error"}}
                self.reply(json.dumps(mes))
            else:
                msg("General Error, traceback: %s"%err_)
                mes = {"gethiscore":{"error":"Internal Error"}}
                self.reply(json.dumps(mes))
        
        if val.has_key("from") and val.has_key("mode")\
          and val.has_key("quest") and len(val.keys())==3:
                self.main.getHiscore(val["quest"], val["mode"])\
                    .addCallback(cbRes).addErrback(cbError)
